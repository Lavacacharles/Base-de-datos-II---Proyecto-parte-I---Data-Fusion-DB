...
void writeFile(string filename){
    ...
    Record record;
    ifstream fileIn("datos.csv");
    char cabecera[40];
    fileIn.getline(cabecera, 40); //Ignoro la primera linea
    while(true){
        record.setData(fileIn); //Parseo la data para cada record
        if(fileIn.eof()) break;
        // record.showData();
        file.insert(record); //Inserto al archivo binario
    }
...
void readFile(string filename){
    ...
    // Record record1 = file.search("Roberto");
    file.getAll();
    cout<<file.getAll().size()<<endl;
    // record.showData();
    // record1.showData();
}
vector<Bucket> getData(string filename){
    StaticHashFile file(filename, N_BUCKETS);
    cout<<"--------- show all data -----------\n";
    // file.scanAll();
    cout<<"--------- search Pancho -----------\n";
...
PYBIND11_MODULE(module_name, m) {
    py::class_<Record>(m, "Record")
        ...
        .def("showData", &Record::showData);

    py::class_<Bucket>(m, "Bucket")
        .def(py::init<>())
        .def_readwrite("records", &Bucket::records)
        .def_readwrite("size", &Bucket::size)
        .def_readwrite("next_bucket", &Bucket::next_bucket)
        .def("showData", &Bucket::showData);

    m.def("getData", &getData, "Get all buckets from the file");
}