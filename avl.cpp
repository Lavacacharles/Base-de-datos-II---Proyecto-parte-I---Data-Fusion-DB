...
template <class R, class T> void test_insert1(AVLFile<R, T> &avl) {
...
template <class R, class T> void test_remove(AVLFile<R, T> &avl) {
  avl.remove(10015);
  avl.viewFile();
...
template <class R, class T> void test_insert2(AVLFile<R, T> &avl) {
  /*Comentando por cambio en struct Record
  avl.viewFile();
  avl.add(Record<int>(10009, "Aaron", "Navaro","" ,1000));
  avl.viewFile();
...
template <class R, class T> void test_insert3(AVLFile<R, T> &avl) {
  /*Comentando por cambio en struct Record
  avl.viewFile();
  avl.add(Record<int>(10015, "Juan", "Aquino","" ,2000));
  avl.viewFile();
...
template <class R, class T> void test_search(AVLFile<R, T> &avl) {
  avl.search(10025);
  avl.viewFile();
}

template <class R, class T> void test_rangeSearch(AVLFile<R, T> &avl) {
...
int main() {
  ...
  test_search(avl);*/
  avl.viewFile();
  buildFromCSV(avl, "datos_m.csv", 1);
  avl.remove("4");
  avl.viewFile(); /*
   for(int i =0; i<642; i++)
        avl.search(i);
